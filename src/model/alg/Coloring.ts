...
/**
 * Idea to use for finding biclique centers.
 * Identical to RLF, but we take the vertex with the lowest degree first. (recursive smallest first)
 *
 * @param G
 * @returns
 */
export function rsfColoring<V>(G: Graph<V>): Set<V>[] {
  G = G.copy();
  const res: Set<V>[] = [];
  while (G.getVertices().length > 0) {
    const S = new Set<V>(); //next color
